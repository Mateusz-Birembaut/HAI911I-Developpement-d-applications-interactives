...
void updateSystem() {
    ...
    unsigned int ncolumns = mesh.V.size() * 3; // nb points * 3 pour x y z
    unsigned int nrows = 0; // nb d'arretes * 2 * 3 pour x y z
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        ...
        for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;   
            ...
            // indice 1 ligne
            // indice 2_1 c'est indices de notre v
            // indice 2_2 c'est indices de notre voisin 
            int indexV {v*3};
    ...
    for (unsigned int i = 0; i < nrows; ++i) {
        for (unsigned int j = 0; j < ncolumns; ++j) {
            std::cout << arapLinearSystem.A(i, j) << " ";
        }
        std::cout << std::endl;
    }
    */
    ...
    for( unsigned int arapIteration = 0 ; arapIteration < maxIterationsForArap ; ++arapIteration ) {
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            if(verticesHandles[v] != -1) {
                ...
                // WHAT TO PUT HERE ??????? How to update the entries of b ?
                arapLinearSystem.b( curRow) = mesh.V[v].p[0];
                arapLinearSystem.b( curRow+1) = mesh.V[v].p[1];
                arapLinearSystem.b( curRow+2) = mesh.V[v].p[2];
                curRow+=3;
            }
